...
class ParsedConstraint {
  final dynamic predicate; // BinaryPredicate, NaryPredicate, or VariableConstraint
...
class VariableSumConstraint extends VariableConstraint {
  ...
  VariableSumConstraint(String targetVariable, List<String> sourceVariables, {this.multipliers})
...
class ExpressionEvaluator {
  ...
  static bool evaluateBoolean(String expression, Map<String, dynamic> variables) {
  ...
  static num _evaluateLeftSide(String expression, Map<String, dynamic> variables) {
  ...
  static num _evaluateRightSide(String expression, Map<String, dynamic> variables) {
    ...
    while (i < expr.length) {
      ...
      if ((char == '+' || char == '-')) {
        ...
        final isNegativeNumber = char == '-' && 
            (i == 0 || (i > 0 && (expr[i-1] == '+' || expr[i-1] == '-' || expr[i-1] == '*' || expr[i-1] == '/')));
...
class ConstraintParser {
  ...
  static ParsedConstraint parseConstraint(String constraintStr, Map<String, List<dynamic>> variableDomains) {
    final cleanStr = constraintStr.trim();
    
    // Extract variable names from the constraint
    final variableNames = _extractVariables(cleanStr, variableDomains.keys.toSet());
    
    // Check if all referenced variables exist
    for (final varName in variableNames) {
      if (!variableDomains.containsKey(varName)) {
        throw ConstraintParseException('Variable "$varName" is not defined', constraintStr);
      }
    }
  ...
  static List<String> _extractVariables(String constraint, Set<String> availableVars) {
    final variables = <String>[];
    
    // First, find all defined variables in the constraint
    final sortedVars = availableVars.toList()..sort((a, b) => b.length.compareTo(a.length));
    
    for (final variable in sortedVars) {
      // Use word boundary regex to match complete variable names
      if (RegExp(r'\b' + RegExp.escape(variable) + r'\b').hasMatch(constraint)) {
    ...
    final allVariableTokens = RegExp(r'\b[A-Za-z_][A-Za-z0-9_]*\b').allMatches(constraint);
  ...
  static ParsedConstraint _parseConstraintLogic(String constraint, List<String> variables, Map<String, List<dynamic>> domains) {
    ...
    if (variables.length == 2) {
      final binaryResult = _parseBinaryVariableConstraint(constraint, variables);
    ...
    if (variables.length == 1) {
      final varConstResult = _parseVariableConstantConstraint(constraint, variables);
    ...
    final arithmeticEqualityResult = _parseArithmeticEquality(constraint, variables);
    if (arithmeticEqualityResult != null) return arithmeticEqualityResult;
    
    // 8. Arithmetic inequality: A + B > 5, A * B <= 20
    final arithmeticInequalityResult = _parseArithmeticInequality(constraint, variables);
  ...
  static ParsedConstraint? _parseBinaryVariableConstraint(String constraint, List<String> variables) {
    ...
    for (final (regex, op) in comparisonPatterns) {
      ...
      if (match != null) {
        ...
        if (variables.contains(matchVar1) && variables.contains(matchVar2)) {
          ...
          return ParsedConstraint(predicate, [matchVar1, matchVar2], ConstraintType.binary);
        }
      }
    }
  ...
  static ParsedConstraint? _parseVariableConstantConstraint(String constraint, List<String> variables) {
  ...
  static ParsedConstraint? _parseArithmeticEquality(String constraint, List<String> variables) {
    final match = RegExp(r'^(.+?)\s*==\s*(-?\d+(?:\.\d+)?)$').firstMatch(constraint);
    ...
    // A + B == 10 (simple sum)
    if (leftSide.contains('+') && !leftSide.contains('*') && !leftSide.contains('/')) {
      final parts = leftSide.split('+').map((s) => s.trim()).toList();
      if (parts.every((part) => variables.contains(part))) {
        return ParsedConstraint(
          variables.length == 2 ? exactSumBinary(rightValue) : exactSum(rightValue),
    ...
    if (leftSide.contains('*') && !leftSide.contains('+') && !leftSide.contains('-')) {
      ...
      if (parts.every((part) => variables.contains(part))) {
        return ParsedConstraint(
          variables.length == 2 ? exactProductBinary(rightValue) : exactProduct(rightValue),
          variables, variables.length == 2 ? ConstraintType.binary : ConstraintType.nary
        );
      }
    }
  ...
  static ParsedConstraint? _parseArithmeticInequality(String constraint, List<String> variables) {
    final patterns = [
      (RegExp(r'^(.+?)\s*>=\s*(-?\d+(?:\.\d+)?)$'), '>='),
      (RegExp(r'^(.+?)\s*>\s*(-?\d+(?:\.\d+)?)$'), '>'),
      (RegExp(r'^(.+?)\s*<=\s*(-?\d+(?:\.\d+)?)$'), '<='),
      ...
      if (match != null) {
        ...
        if (leftSide.contains('+')) {
          switch (op) {
            case '>=':
              return ParsedConstraint(
                variables.length == 2 ? minSumBinary(rightValue) : minSum(rightValue),
                variables, variables.length == 2 ? ConstraintType.binary : ConstraintType.nary
              );
            case '>':
              return ParsedConstraint(
                variables.length == 2 ? minSumBinary(rightValue + 0.001) : minSum(rightValue + 0.001),
                variables, variables.length == 2 ? ConstraintType.binary : ConstraintType.nary
              );
            case '<=':
              return ParsedConstraint(
                variables.length == 2 ? maxSumBinary(rightValue) : maxSum(rightValue),
                variables, variables.length == 2 ? ConstraintType.binary : ConstraintType.nary
              );
            case '<':
              return ParsedConstraint(
                variables.length == 2 ? maxSumBinary(rightValue - 0.001) : maxSum(rightValue - 0.001),
                variables, variables.length == 2 ? ConstraintType.binary : ConstraintType.nary
              );
          }
        }
        ...
        if (leftSide.contains('*')) {
          switch (op) {
            case '>=':
              return ParsedConstraint(
                variables.length == 2 ? minProductBinary(rightValue) : minProduct(rightValue),
                variables, variables.length == 2 ? ConstraintType.binary : ConstraintType.nary
              );
            case '>':
              return ParsedConstraint(
                variables.length == 2 ? minProductBinary(rightValue + 0.001) : minProduct(rightValue + 0.001),
                variables, variables.length == 2 ? ConstraintType.binary : ConstraintType.nary
              );
            case '<=':
              return ParsedConstraint(
                variables.length == 2 ? maxProductBinary(rightValue) : maxProduct(rightValue),
                variables, variables.length == 2 ? ConstraintType.binary : ConstraintType.nary
              );
            case '<':
              return ParsedConstraint(
                variables.length == 2 ? maxProductBinary(rightValue - 0.001) : maxProduct(rightValue - 0.001),
                variables, variables.length == 2 ? ConstraintType.binary : ConstraintType.nary
              );
          }
        }
  ...
  static ParsedConstraint? _parseVariableEquation(String constraint, List<String> variables) {
    final match = RegExp(r'^(.+?)\s*==\s*([A-Za-z_][A-Za-z0-9_]*)$').firstMatch(constraint);
    if (match == null) return null;
    ...
    if (variables.contains(rightVar)) {
      ...
      if (leftSide.contains('+')) {
        ...
        return ParsedConstraint(sumConstraint, variables, ConstraintType.variableSum);
      }
      
      if (leftSide.contains('*')) {
        final productConstraint = VariableProductConstraint(rightVar, leftVars);
        return ParsedConstraint(productConstraint, variables, ConstraintType.variableProduct);
      }
    }
  ...
  static ParsedConstraint? _parseRangeConstraint(String constraint, List<String> variables) {
    final rangeMatch = RegExp(r'^(-?\d+(?:\.\d+)?)\s*<=\s*(.+?)\s*(<=|<)\s*(-?\d+(?:\.\d+)?)$').firstMatch(constraint);
    ...
    if (expression.contains('+')) {
      ...
      return ParsedConstraint(
        variables.length == 2 ? sumInRangeBinary(minVal, actualMax) : sumInRange(minVal, actualMax),
        variables, variables.length == 2 ? ConstraintType.binary : ConstraintType.nary
      );
    }
  ...
  static ParsedConstraint? _parseOrderingConstraint(String constraint, List<String> variables) {
    if (constraint.contains('<') && !constraint.contains('<=') && !constraint.contains('==')) {
      ...
      if (parts.length > 2 && parts.every((part) => variables.contains(part))) {
        return ParsedConstraint(strictlyAscendingInOrder(parts), variables, ConstraintType.nary);
      }
    } else if (constraint.contains('<=') && !constraint.contains('==')) {
      final parts = constraint.split('<=').map((s) => s.trim()).toList();
      if (parts.length > 2 && parts.every((part) => variables.contains(part))) {
        return ParsedConstraint(ascendingInOrder(parts), variables, ConstraintType.nary);
      }
    }
  ...
  static ParsedConstraint _createExpressionConstraint(String constraint, List<String> variables) {
